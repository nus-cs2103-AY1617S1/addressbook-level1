...
public class AddressBook {
	...
	private static final String COMMAND_EDIT_DESC = "Edits a person's information in the address book, based on person index number.";
	...
	private static String executeEditPersons(String commandArgs) {
		...
		String personDetails = getUserInput();
		final Optional<String[]> decodeResult = decodePersonFromString(personDetails);
		if (!decodeResult.isPresent()) {
			return getMessageForInvalidCommandInput(COMMAND_ADD_WORD, getUsageInfoForAddCommand());
		}
		final String[] personToEdit = decodeResult.get();
		ALL_PERSONS.set(index - 1, personToEdit);

		return getMessageForSuccessfulAddPerson(personToEdit);
	}
