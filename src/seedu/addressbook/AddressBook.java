...
public class AddressBook {
	...
	private static final String MESSAGE_EDIT_PERSON_SUCCESS = "Modified Person: %1$s";
	private static final String MESSAGE_DELETE_PERSON_SUCCESS = "Deleted Person: %1$s"; 
	private static final String MESSAGE_DISPLAY_PERSON_DATA = "%1$s  Phone Number: %2$s  Email: %3$s";
	private static final String MESSAGE_DISPLAY_LIST_ELEMENT_INDEX = "%1$d. ";
	private static final String MESSAGE_GOODBYE = "Exiting Address Book... Good bye!";
	private static final String MESSAGE_INVALID_COMMAND_FORMAT = "Invalid command format: %1$s " + LINE_SEPERATOR
			+ "%2$s";
	...
	private static final String MESSAGE_INVALID_PROGRAM_ARGS = "Too many parameters! Correct program argument format:"
			+ LINE_SEPERATOR + "\tjava AddressBook" + LINE_SEPERATOR + "\tjava AddressBook [custom storage file path]";
	...
	private static final String COMMAND_EDIT_DESC = "Edits a person in the address book.";
	private static final String COMMAND_EDIT_PARAMETERS = "INDEX NAME " + PERSON_DATA_PREFIX_PHONE + "PHONE_NUMBER "
			+ PERSON_DATA_PREFIX_EMAIL + "EMAIL";
	 ...
	 * List<HashMap<PersonProperty,String>> ALL_PERSONS = new ArrayList<>() That
	 * is because List is an interface implemented by the ArrayList class. In
	 * this code we use ArrayList instead because we wanted to to stay away from
	 * advanced concepts such as interface inheritance.
	 * =========================================================================
	 * ===========================
	 */
	...
	private static String executeEditPerson(String commandArgs) {
		try {
			...
			String personIndex = commandArgs.substring(0, commandArgs.indexOf(' '));

			if (!isPersonArgsValid(personIndex)) {
				return getMessageForInvalidCommandInput(COMMAND_EDIT_WORD, getUsageInfoForEditCommand());
			}
			
			final int targetVisibleIndex = extractTargetIndexFromPersonArgs(personIndex);
			if (!isDisplayIndexValidForLastPersonListingView(targetVisibleIndex)) {
				return MESSAGE_INVALID_PERSON_DISPLAYED_INDEX;
			}
			String encodedPerson = commandArgs.substring(commandArgs.indexOf(' '));
			// try decoding a person from the raw args
			final Optional<HashMap<PersonProperty, String>> decodeResult = decodePersonFromString(encodedPerson);

			// checks if args are valid (decode result will not be present if
			...
			// add the person as specified
			final HashMap<PersonProperty, String> modifiedPersonDetails = decodeResult.get();

			editPersonInAddressBook(targetVisibleIndex, modifiedPersonDetails);

			return getMessageForSuccessfulEdit(modifiedPersonDetails);
	...
	/**
	 * Checks validity of person argument string's format.
	 *
	 * @param rawArgs
	 ...
	 *
	 * @param rawArgs
	 *            raw command args string for the person command
	 ...
	 */
	private static String getMessageForSuccessfulEdit(HashMap<PersonProperty, String> newPersonDetails) {
		return String.format(MESSAGE_EDIT_PERSON_SUCCESS,getMessageForFormattedPersonData(newPersonDetails));
	}
	...
	/**
	 * Edits a person in the address book, target is identified by it's absolute
	 * index in the full list. Saves changes to storage file.
	 *
	 * @param index
	 *            absolute index of person to edit (index within
	 *            {@link #ALL_PERSONS})
	 */
	private static void editPersonInAddressBook(int index, HashMap<PersonProperty, String> newPersonDetails) {
		HashMap<PersonProperty, String> personToEdit = ALL_PERSONS.get(index - DISPLAYED_INDEX_OFFSET);
		personToEdit.put(PersonProperty.NAME, newPersonDetails.get(PersonProperty.NAME));
		personToEdit.put(PersonProperty.PHONE, newPersonDetails.get(PersonProperty.PHONE));
		personToEdit.put(PersonProperty.EMAIL, newPersonDetails.get(PersonProperty.EMAIL));
	...
	private static String getUsageInfoForAllCommands() {
		return getUsageInfoForAddCommand() + LINE_SEPERATOR + getUsageInfoForFindCommand() + LINE_SEPERATOR
				+ getUsageInfoForViewCommand() + LINE_SEPERATOR + getUsageInfoForEditCommand() + LINE_SEPERATOR
				+ getUsageInfoForDeleteCommand() + LINE_SEPERATOR + getUsageInfoForClearCommand() + LINE_SEPERATOR
				+ getUsageInfoForExitCommand() + LINE_SEPERATOR + getUsageInfoForHelpCommand();
	}
	...
	private static String getUsageInfoForEditCommand() {
		return String.format(MESSAGE_COMMAND_HELP, COMMAND_EDIT_WORD, COMMAND_EDIT_DESC) + LINE_SEPERATOR
				+ String.format(MESSAGE_COMMAND_HELP_PARAMETERS, COMMAND_EDIT_PARAMETERS) + LINE_SEPERATOR
				+ String.format(MESSAGE_COMMAND_HELP_EXAMPLE, COMMAND_EDIT_EXAMPLE) + LINE_SEPERATOR;
	}
