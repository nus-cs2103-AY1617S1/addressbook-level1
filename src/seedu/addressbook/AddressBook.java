...
public class AddressBook {
	...
	/**
	 * Splits raw user input into command word and command arguments stringx
	 *
	...
	private static String getMessageForPersonsDisplayedSummary(
			ArrayList<HashMap<PersonProperty, String>> personsFound) {
	...
	private static boolean displayIndexNotValidForLastPersonListing(final int targetVisibleIndex) {
		return !isDisplayIndexValidForLastPersonListingView(targetVisibleIndex);
	}

	private static boolean deletedPersonNotValid(String commandArgs) {
		return !isDeletePersonArgsValid(commandArgs);
	}
	 ...
	 */
	private static String getMessageForSuccessfulDelete(HashMap<PersonProperty, String> targetInModel) {
		return String.format(MESSAGE_DELETE_PERSON_SUCCESS, getMessageForFormattedPersonData(targetInModel));
	}
	 ...
	 *            file for saving
	 */
	private static void savePersonsToFile(ArrayList<HashMap<PersonProperty, String>> arrayList, String filePath) {
		final ArrayList<String> linesToWrite = encodePersonsToStrings(arrayList);
		try {
	 ...
	 *            to add
	 */
	private static void addPersonToAddressBook(HashMap<PersonProperty, String> personToAdd) {
	 ...
	 */
	private static boolean isPersonDeletedFromAddressBook(HashMap<PersonProperty, String> exactPerson) {
	 ...
	 */
	private static ArrayList<String> encodePersonsToStrings(ArrayList<HashMap<PersonProperty, String>> arrayList) {
		final ArrayList<String> encoded = new ArrayList<>();
		for (HashMap<PersonProperty, String> person : arrayList) {
	...
	private static boolean isPersonDataExtractableFrom(String personData) {
		...
		return isArgumentsNumberCorrect(splitArgs) // 3 arguments
				&& isFirstArgumentEmpty(splitArgs) // non-empty arguments
				&& !splitArgs[1].isEmpty() && !splitArgs[2].isEmpty();
	}

	private static boolean isFirstArgumentEmpty(final String[] splitArgs) {
		return !splitArgs[0].isEmpty();
	}

	private static boolean isArgumentsNumberCorrect(final String[] splitArgs) {
	 ...
	 */
	private static boolean isPersonDataValid(HashMap<PersonProperty, String> decodedPerson) {
		return isPersonNameValid(decodedPerson.get(PersonProperty.NAME))
				&& isPersonPhoneValid(decodedPerson.get(PersonProperty.PHONE))
				&& isPersonEmailValid(decodedPerson.get(PersonProperty.EMAIL));
	}
