...
public class AddressBook {
	...
	public static void main(String[] args) {
		...
		if (args.length == 0) {
			System.out.println("|| " + MESSAGE_USING_DEFAULT_FILE);
		...
		while (true) {
			...
			if (commandType.equals("add")) {
				...
				if (!decodeResult.isPresent()) {
					feedback = String.format(MESSAGE_INVALID_COMMAND_FORMAT, "add",
							String.format("%1$s: %2$s", "add", COMMAND_ADD_DESC) + "\r\n|| "
									+ String.format("\tParameters: %1$s", COMMAND_ADD_PARAMETERS) + "\r\n|| "
									+ String.format("\tExample: %1$s", COMMAND_ADD_EXAMPLE) + "\r\n|| ");
				} else {
					...
					try {
						...
						System.out.println("|| " + String.format(MESSAGE_ERROR_WRITING_TO_FILE, storageFilePath));
						System.out.println("|| " + MESSAGE_GOODBYE);
						System.out.println("|| " + "===================================================");
						System.out.println("|| " + "===================================================");
						System.exit(0);
					}
					feedback = String.format(MESSAGE_ADDED, personToAdd.get(PersonProperty.NAME),
				...
				for (HashMap<PersonProperty, String> person : ALL_PERSONS) {
					final Set<String> wordsInName = new HashSet<>((ArrayList<String>) new ArrayList(Arrays.asList(person.get(PersonProperty.NAME).trim().split("\\s+"))));
				...
				for (int i = 0; i < personsFound.size(); i++) {
					...
					messageAccumulator.append('\t').append(String.format(MESSAGE_DISPLAY_LIST_ELEMENT_INDEX, displayIndex)
					+ String.format(MESSAGE_DISPLAY_PERSON_DATA, person1.get(PersonProperty.NAME),
				...
				feedback = String.format(MESSAGE_PERSONS_FOUND_OVERVIEW, personsFound.size());
				...
				for (int i = 0; i < toBeDisplayed.size(); i++) {
					...
					final int displayIndex = i + DISPLAYED_INDEX_OFFSET;
					messageAccumulator.append('\t').append(String.format(MESSAGE_DISPLAY_LIST_ELEMENT_INDEX, displayIndex)
					+ String.format(MESSAGE_DISPLAY_PERSON_DATA, person.get(PersonProperty.NAME),
				...
				feedback = String.format("%1$s: %2$s", "add", COMMAND_ADD_DESC) + "\r\n|| "
						+ String.format("\tParameters: %1$s", COMMAND_ADD_PARAMETERS) + "\r\n|| "
						+ String.format("\tExample: %1$s", COMMAND_ADD_EXAMPLE) + "\r\n|| " + "\r\n|| "
						+ String.format("%1$s: %2$s", "find", COMMAND_FIND_DESC) + "\r\n|| "
						+ String.format("\tParameters: %1$s", COMMAND_FIND_PARAMETERS) + "\r\n|| "
						+ String.format("\tExample: %1$s", COMMAND_FIND_EXAMPLE) + "\r\n|| " + "\r\n|| "
						+ String.format("%1$s: %2$s", "list", COMMAND_LIST_DESC) + "\r\n|| "
						+ String.format("\tExample: %1$s", COMMAND_LIST_EXAMPLE) + "\r\n|| " + "\r\n|| "
						+ String.format("%1$s: %2$s", "delete", COMMAND_DELETE_DESC) + "\r\n|| "
						+ String.format("\tParameters: %1$s", COMMAND_DELETE_PARAMETER) + "\r\n|| "
						+ String.format("\tExample: %1$s", COMMAND_DELETE_EXAMPLE) + "\r\n|| " + "\r\n|| "
						+ String.format("%1$s: %2$s", "clear", COMMAND_CLEAR_DESC) + "\r\n|| "
						+ String.format("\tExample: %1$s", COMMAND_CLEAR_EXAMPLE) + "\r\n|| " + "\r\n|| "
						+ String.format("%1$s: %2$s", "exit", COMMAND_EXIT_DESC)
						+ String.format("\tExample: %1$s", COMMAND_EXIT_EXAMPLE) + "\r\n|| "
						+ String.format("%1$s: %2$s", "help", COMMAND_HELP_DESC)
						+ String.format("\tExample: %1$s", COMMAND_HELP_EXAMPLE);
				...
				feedback = String.format(MESSAGE_INVALID_COMMAND_FORMAT, commandType,
						String.format("%1$s: %2$s", "add", COMMAND_ADD_DESC) + "\r\n|| "
								+ String.format("\tParameters: %1$s", COMMAND_ADD_PARAMETERS) + "\r\n|| "
								+ String.format("\tExample: %1$s", COMMAND_ADD_EXAMPLE) + "\r\n|| "
								+ "\r\n|| " + String.format("%1$s: %2$s", "find", COMMAND_FIND_DESC) + "\r\n|| "
	...
	private static String executeDeletePerson(String commandArgs) {
		if (!isDeletePersonArgsValid(commandArgs)) {
			return String.format(MESSAGE_INVALID_COMMAND_FORMAT, "delete",
					String.format("%1$s: %2$s", "delete", COMMAND_DELETE_DESC) + "\r\n|| "
							+ String.format("\tParameters: %1$s", COMMAND_DELETE_PARAMETER) + "\r\n|| "
							+ String.format("\tExample: %1$s", COMMAND_DELETE_EXAMPLE) + "\r\n|| ");
		}
		...
		if (deletePersonFromAddressBook(targetInModel)) {
			return String.format(MESSAGE_DELETE_PERSON_SUCCESS, String.format(MESSAGE_DISPLAY_PERSON_DATA, targetInModel.get(PersonProperty.NAME),
	...
	private static void createFileIfMissing(String filePath) {
		...
		try {
			...
			System.out.println("|| " + String.format(MESSAGE_ERROR_CREATING_STORAGE_FILE, filePath));
			System.out.println("|| " + MESSAGE_GOODBYE);
			System.out.println("|| " + "===================================================");
			System.out.println("|| " + "===================================================");
			System.exit(0);
	...
	private static ArrayList<HashMap<PersonProperty, String>> loadPersonsFromFile(String filePath) {
		...
		if (!successfullyDecoded.isPresent()) {
			System.out.println("|| " + MESSAGE_INVALID_STORAGE_FILE_CONTENT);
			System.out.println("|| " + MESSAGE_GOODBYE);
			System.out.println("|| " + "===================================================");
			System.out.println("|| " + "===================================================");
			System.exit(0);
	...
	private static ArrayList<String> getLinesInFile(String filePath) {
		...
		try {
			...
			System.out.println("|| " + String.format(MESSAGE_ERROR_READING_FROM_FILE, filePath));
			System.out.println("|| " + MESSAGE_GOODBYE);
			System.out.println("|| " + "===================================================");
			System.out.println("|| " + "===================================================");
			System.exit(0);
	...
	private static ArrayList<String> encodePersonsToStrings(ArrayList<HashMap<PersonProperty, String>> persons) {
		...
		for (HashMap<PersonProperty, String> person : persons) {
			encoded.add(String.format(PERSON_STRING_REPRESENTATION, person.get(PersonProperty.NAME),
					person.get(PersonProperty.PHONE), person.get(PersonProperty.EMAIL)));
		}
		return encoded;
	}
	...
	private static Optional<HashMap<PersonProperty, String>> decodePersonFromString(String encoded) {
		...
		&& decodedPerson.get(PersonProperty.EMAIL).matches("\\S+@\\S+\\.\\S+") ? Optional.of(decodedPerson) : Optional.empty();
	}
