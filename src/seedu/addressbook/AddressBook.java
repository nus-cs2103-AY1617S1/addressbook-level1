...
public class AddressBook {
    ...
    */
    ...
    private static void createFileIfMissing(String filePath) {
        ...
        String[] firstArgument = {String.format(MESSAGE_ERROR_MISSING_STORAGE_FILE, filePath)};
        showToUser(firstArgument);

        try {
            storageFile.createNewFile();
            String[] secondArgument = {String.format(MESSAGE_STORAGE_FILE_CREATED, filePath)};
            showToUser(secondArgument);
        } catch (IOException ioe) {
        	String[] thirdArgument = {String.format(MESSAGE_ERROR_CREATING_STORAGE_FILE, filePath)};
            showToUser(thirdArgument);
            exitProgram();
        }
    }
    ...
    private static ArrayList<HashMap<PersonProperty,String>> loadPersonsFromFile(String filePath) {
        ...
        if (!successfullyDecoded.isPresent()) {
        	String[] argument = {MESSAGE_INVALID_STORAGE_FILE_CONTENT};
            showToUser(argument);
            exitProgram();
        }
    ...
    private static ArrayList<String> getLinesInFile(String filePath) {
        ...
        try {
        	...
        	String[] argument = {String.format(MESSAGE_ERROR_MISSING_STORAGE_FILE, filePath)};
            showToUser(argument);
            exitProgram();
        } catch (IOException ioe) {
        	String[] argument = {String.format(MESSAGE_ERROR_READING_FROM_FILE, filePath)};
            showToUser(argument);
            exitProgram();
        }
        return lines;
    ...
    private static void savePersonsToFile(ArrayList<HashMap<PersonProperty,String>> persons, String filePath) {
        ...
        try {
        	...
        	String[] argument = {String.format(MESSAGE_ERROR_WRITING_TO_FILE, filePath)};
            showToUser(argument);
            exitProgram();
        }
    }
    ...
    private static HashMap<PersonProperty,String> makePersonFromData(String name, String phone, String email) {
    	...
    	final HashMap<PersonProperty,String> person = new HashMap<PersonProperty,String>(3);
    	person.put(PersonProperty.NAME,name);
    	person.put(PersonProperty.PHONE, phone);
    	person.put(PersonProperty.EMAIL, email);
    	return person;
    	/*
    	final HashMap<PersonProperty,String> person = new String[PERSON_DATA_COUNT];
